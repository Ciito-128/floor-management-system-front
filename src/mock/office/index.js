...
function generateWorkstations(office) {
  ...
  for (let i = 1; i <= office.capacity; i++) {
    workstations.push({
      ...
      userId: office.usageStatus ? `1234${office.officeId}-${i}` : null, // 工位使用人ID，初始为空
      userName: office.usageStatus ? `员工${office.officeId}-${i}` : null, // 工位使用人名称，初始为空
      stationPhone:  office.usageStatus ? `1921234516${i}`: null, // 电话，初始为空
    });
  }
  return workstations;
}
